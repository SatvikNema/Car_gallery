...
router.post("/",function(req, res){
	...
	company.findOne({comp_name: comp}).populate("car_models").exec(function(err, company){
		if(err)
		{
			console.log(err)
		}else{
...
router.get("/model_select/:id", function(req, res){
	...
	company_model.findById(modelid).populate("comments").exec(function(err, found_mod){
		if(err){
			console.log(err);
		}else
		{
